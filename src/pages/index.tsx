...
// MailerLite API helper functions for merging data
...
const getExistingSubscriber = async (email: string) => {
  try {
...
const mergeFields = (existingFields: Record<string, any> = {}, newFields: Record<string, any>) => {
...
const addSubscriberWithMerge = async (email: string, fields: Record<string, any>, groups: string[] = ["168614688429442382"]) => {
  ...
  const mergedFields = mergeFields(existingFields, fields);
  
  const existingGroups = existingSubscriber?.groups?.map((g: any) => g.id?.toString() || g.toString()) || [];
  const mergedGroups = mergeArrays(existingGroups, groups);
  
  const existingTags = existingSubscriber?.tags || [];
  const mergedTags = mergeArrays(existingTags, ['testing-fantasy', 'workshop-attendee']);
  
  const payload = {
    email: email,
    name: existingSubscriber?.name || email.split('@')[0],
    fields: mergedFields,
    groups: mergedGroups,
    tags: mergedTags
  };
...
function HomepageHeader() {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await addSubscriberWithMerge(
        email,
        {
